...
import re
from utils import mage_preprocess
...
def mage_detect(input_text, tokenizer, model, device='cuda:0', th=-3.08583984375):
    label2decisions = {
        0: "machine-generated",
        1: "human-written",
    }
    input_text = mage_preprocess(input_text)
    ...
    return label2decisions[decision] #, outputs.logits.detach().cpu().tolist()[0]


def radar_detect(input_text, tokenizer, model, device):
    import torch.nn.functional as F
    with torch.no_grad():
        inputs = tokenizer(input_text, padding=True, truncation=True, max_length=512, return_tensors="pt")
        inputs = {k:v.to(device) for k,v in inputs.items()}
        output_probs = F.log_softmax(model(**inputs).logits,-1)[:,0].exp().tolist()[0]
        if output_probs >= 0.5:
           dec = "machine-generated"
        else:
           dec = "human-written"
        
        return dec # , output_probs

def detectaive_detect(input_text, tokenizer, model, device):
    TEXT_CLASS_MAPPING = {
        2: 'machine-generated',
        0: 'human-Written',
    ...
    with torch.no_grad():
        model_inputs = tokenizer(input_text, padding=True, truncation=True, max_length=512, return_tensors="pt").to(device)
        outputs = model(**model_inputs)

        decision = TEXT_CLASS_MAPPING[outputs.logits.argmax().item()]
...
def get_detectors(device):
    ...
    detectaive_clf = AutoModelForSequenceClassification.from_pretrained("raj-tomar001/LLM-DetectAIve_deberta-base").to(device)
    detectaive_tokenizer = AutoTokenizer.from_pretrained("raj-tomar001/LLM-DetectAIve_deberta-base")

    return mage_clf, mage_tokenizer, radar_clf, radar_tokenizer, detectaive_clf, detectaive_tokenizer
...
def main(args):
    # DEVICE = "cuda:0"
    # datapath = "test-xsum-250107.json"
    # datapath = "profiling_ilc/data/profiling_data.ctrl_and_dpo.500test.json"

    DEVICE = args.device
    datapath = args.datapath
    data = get_data(datapath)
    
    mage_clf, mage_tokenizer, radar_clf, radar_tokenizer, detectaive_clf, detectaive_tokenizer = get_detectors(device=DEVICE)
    
    for elem in tqdm(data):
        for target in ["human", "llama", "dpo-llama", "ctrl", "verbalized-llama"]:
            if f"{target}-mage-pred" not in elem.keys():
                mage_pred = mage_detect(elem[target], tokenizer=mage_tokenizer, model=mage_clf, device=DEVICE) 
            else:
                mage_pred = elem[f"{target}-mage-pred"]
            if f"{target}-radar-pred" not in elem.keys():
                radar_pred = radar_detect(elem[target], tokenizer=radar_tokenizer, model=radar_clf, device=DEVICE)
            else:
                radar_pred = elem[f"{target}-radar-pred"]
            if f"{target}-detectaive-pred" not in elem.keys():
                detectaive_pred = detectaive_detect(elem[target], tokenizer=detectaive_tokenizer, model=detectaive_clf, device=DEVICE)
            else:
                detectaive_pred = elem[f"{target}-detectaive-pred"]
            
            elem.update(
                {f"{target}-mage-pred": mage_pred,
                 f"{target}-radar-pred": radar_pred,
                 f"{target}-detectaive-pred": detectaive_pred,
                 }
                 )
    with open(datapath, "w") as jf:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datapath", type=str, default="data/dpo-generation/test-xsum-250107.json")
